  ...
  return (
    ...
    >
      ...
      >
        {/* FRONT */}
        <div 
          className="absolute inset-0 bg-slate-50 border border-slate-200/50 rounded-2xl p-4 flex flex-col justify-between group-hover:shadow-md transition-shadow [backface-visibility:hidden]"
        >
          <div className="flex justify-between items-start">
            ...
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity duration-300">Tap to flip</div>
          </div>
          <div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 p-5 md:p-8 hover:shadow-md transition-all duration-500">
          ...
          <div className="flex items-center gap-4 md:gap-8 mb-6 md:mb-8">
            ...
            <div>
              ...
              <div className="text-[10px] text-slate-500 font-mono tracking-wider bg-slate-100 px-2 py-1 rounded border border-slate-200 inline-block">
                SYS: <span className="text-emerald-500 font-bold">ON</span> | {clockTime || '—'} UTC
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {/* Primary Sensor Card */}
            <div className="col-span-2 bg-slate-50 border border-slate-200/50 rounded-2xl p-4 flex items-center justify-between hover:shadow-sm transition-all duration-300 cursor-default">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm font-bold text-slate-700">SENTINEL-2 ORBITAL</div>
                </div>
              </div>
              <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-xs font-bold uppercase tracking-wider border border-emerald-200/50">Online</div>
            </div>
            ...
            {(() => {
              ...
              return status ? (
                ...
                <div className="bg-slate-50 border border-slate-200/50 rounded-2xl p-4 flex flex-col justify-between h-28 cursor-default">
                  <div className="flex justify-between items-start">
        ...
        <AnimatePresence mode="wait">
          {!scanResult && !loading && !error && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="rounded-3xl border p-7 mb-2 bg-slate-50 border-slate-200">
                <h2 className="font-bold uppercase tracking-widest text-xs flex items-center gap-2 text-slate-700 mb-3">
                  <Globe weight="bold" className="w-4 h-4" /> System Explanation
                </h2>
                <p className="text-sm font-medium text-slate-600 leading-relaxed mb-4">
                  Welcome to the KABAW Orbital Crop & Land Monitoring System. This dashboard connects directly to the <strong className="text-slate-900">Sentinel-2 Satellite</strong> API to provide real-time agricultural telemetry.
                </p>
          ...
          {error && !loading && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="rounded-3xl border p-7 mb-2 bg-rose-50 border-rose-200">
                <h2 className="font-bold uppercase tracking-widest text-xs flex items-center gap-2 text-rose-700 mb-2">
          ...
          {scanResult && !loading && !error && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="rounded-3xl border p-6 mb-2 bg-slate-50 border-slate-200 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-emerald-500/20 flex items-center justify-center">
                    <Globe weight="duotone" className="w-5 h-5 text-emerald-600" />
                  </div>
                  <div>
                    <h2 className="font-bold uppercase tracking-widest text-xs text-slate-700">Telemetry Acquired</h2>
                    <p className="text-[10px] text-slate-500 font-medium">Click the score cards above to view detailed analysis</p>
                  </div>
                </div>
                <div className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-bold uppercase tracking-wider">Optimal Signal</div>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
